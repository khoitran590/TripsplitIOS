...
| --- | --- | --- | --- |
| SEC-01 | P0 | Enforce financial and trip authorization on the server | Applied to linked project; staging authorization tests pending |
...
| Storage | Attachment ACL table/registration RPC, private scoped reads, 5 MB JPEG bucket cap, per-user object/byte quotas, blocked-avatar/feed-media handling, orphan cleanup on failed registration | [storage migration](supabase/migrations/20260802010000_storage_authorization.sql), [policy boundary fix](supabase/migrations/20260803000000_storage_attachment_policy_boundary.sql), and [ReceiptService.swift](Tripsplit/ReceiptService.swift) applied to the linked project; add decoded file-signature/dimension validation and staging isolation tests |
...
| Invitations | Generic email response, pending-only membership, 72-hour hashed single-use tokens, authenticated email delivery function, preview plus explicit Join/Decline, owner revocation, no auto-accept | [invitation migration](supabase/migrations/20260802040000_invitation_hardening.sql) applied after qualifying Supabase's `extensions` crypto functions, [send-invitation Edge Function](supabase/functions/send-invitation/index.ts); configure Resend and replace fallback links with universal links |
...
1. Complete for the currently linked project: its existing baseline was recorded as `20260701000000` without replaying baseline SQL, and the five August migrations were applied in order. Keep backup/recovery and schema-diff evidence before future production changes.
2. Run a clean local reset and `supabase test db`, then execute the multi-identity matrix below. This workspace still does not have a usable local Docker/Postgres runtime, so clean-reset and pgTAP execution are not yet evidenced.
