-- Settlement parties are billing participants from the authoritative trip metadata,
-- not necessarily authenticated collaborators in trip_members. This matches expense
-- validation and lets an account-backed member settle with a manually added tripmate.
-- Also make upsert retries idempotent: a lost response must not turn a repeated
-- pending/confirmed write into an invalid same-status transition.
...
begin
    ...
    if tg_op = 'INSERT' then
        ...
        -- INSERT ... ON CONFLICT runs the INSERT trigger first, so a retry arrives here
        -- with a fresh server timestamp. Compare only the immutable financial identity
        -- and lifecycle state; returning OLD also discards any untrusted payload change.
