-- Claude becomes the primary receipt-image processor, with Gemini as fallback.
-- A new receipt-only consent version is enforced by the client and Edge Function; existing
-- Google-only grants therefore cannot authorize a transfer to Anthropic.
