-- TripSplit — versioned baseline schema for cloud-syncing trips.
--
-- Fresh environments apply this file first through `supabase db reset`/`db push`, then
-- apply every later migration in timestamp order. Existing projects that previously ran
-- the former root `supabase_schema.sql` should verify schema parity and mark this one
-- baseline version as applied before pushing the later migrations.
--
